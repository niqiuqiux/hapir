...
pub struct RunnerState {
    ...
    pub shutdown_tx: Arc<Mutex<Option<oneshot::Sender<ShutdownSource>>>>,
    pub shutdown_tx_mpsc: Arc<Mutex<Option<mpsc::Sender<ShutdownSource>>>>,
...
impl RunnerState {
    pub fn new(shutdown_tx: oneshot::Sender<ShutdownSource>) -> Self {
        Self {
    ...
    pub fn new_mpsc(shutdown_tx: mpsc::Sender<ShutdownSource>) -> Self {
        Self {
            sessions: Arc::new(Mutex::new(HashMap::new())),
...
async fn maybe_cleanup_worktree(
    ...
    if let Some(pid) = pid
        && hapir_infra::persistence::is_process_alive(pid)
    {
        info!(pid = pid, "skipping worktree cleanup; child still running");
        return;
    }
    if let Err(e) = crate::worktree::remove_worktree(&wt.base_path, &wt.worktree_path).await {
...
pub async fn do_spawn_session(
    ...
    if session_type == "worktree" {
        match crate::worktree::create_worktree(&req.directory, req.worktree_name.as_deref()).await {
            Ok(wt) => {
    ...
    if !exe_exists || !dir_exists {
        let detail = match (exe_exists, dir_exists) {
            ...
            (true, false) => format!("Directory '{}' does not exist", spawn_directory),
            _ => unreachable!(),
        };
    ...
    let (awaiter_tx, awaiter_rx) = oneshot::channel::<TrackedSession>();
