...
pub async fn check_runner_alive(state_path: &Path, lock_path: &Path) -> Option<u16> {
...
pub async fn spawn_session(port: u16, req: &SpawnSessionRequest) -> Result<serde_json::Value> {
    let client = http_client();
    let resp = client
...
pub async fn is_runner_running_current_version(
    ...
    let current_mtime = crate::orchestrator::get_cli_mtime_ms();
