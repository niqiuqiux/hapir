...
pub fn exponential_backoff_delay(
    ...
    let max_ret = min_delay_ms as f64 + (range as f64 / max_failure_count as f64) * capped as f64;
...
pub async fn with_retry<T, F, Fut, S>(f: F, options: RetryOptions<S>) -> Result<T>
where
    F: Fn() -> Fut,
    ...
    loop {
        match f().await {
            ...
            Err(error) => {
                ...
                let exp_delay = options.min_delay_ms as f64 * 2f64.powi((attempt - 1) as i32);
                let capped = exp_delay.min(options.max_delay_ms as f64);
                let jitter = rand::thread_rng().gen_range(0.0..=0.3) * capped;
                let next_delay = Duration::from_millis((capped + jitter).round() as u64);
