...
impl WsMachineClient {
            ...
            self.ws.on_connect(move || {
                ...
                tokio::spawn(async move {
                    ...
                    if let Some(ref metadata) = *md.lock().await {
                        ...
                        let _ = ws.emit_with_ack("machine-update-metadata", json!({
                            "machineId": mid,
                    ...
                    if let Some(ref state) = *rs.lock().await {
                        ...
                        let _ = ws.emit_with_ack("machine-update-state", json!({
                            "machineId": mid,
                            "expectedVersion": version,
                            "runnerState": state,
                        })).await;
        ...
        let handle = tokio::spawn(async move {
            ...
            loop {
                ...
                let time = std::time::SystemTime::now()
                    ...
                    .unwrap_or_default()
                    .as_millis() as i64;
                ws.emit("machine-alive", json!({ "machineId": mid, "time": time })).await;
            }
        });
        ...
        F: FnOnce(Value) -> Value,
    {
        ...
        let ack = self.ws.emit_with_ack("machine-update-metadata", json!({
        ...
        F: FnOnce(Value) -> Value,
    {
        ...
        let ack = self.ws.emit_with_ack("machine-update-state", json!({
            "machineId": self.machine_id,
            "expectedVersion": version,
            "runnerState": updated,
        })).await?;
