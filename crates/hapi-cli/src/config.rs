...
impl Configuration {
    ...
    pub fn create() -> anyhow::Result<Self> {
        ...
        // Home directory: HAPI_HOME env > ~/.hapi
        let home_dir = if let Ok(home) = std::env::var("HAPI_HOME") {
        ...
        let is_experimental = matches!(
            std::env::var("HAPI_EXPERIMENTAL").as_deref(),
            Ok("true") | Ok("1") | Ok("yes")
        );
    ...
    pub fn load_with_settings(&mut self) -> anyhow::Result<()> {
        ...
        if std::env::var("HAPI_API_URL").is_err() {
