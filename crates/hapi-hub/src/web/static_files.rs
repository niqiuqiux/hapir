    ...
    (
        ...
        Html(
            "<h1>Frontend not built</h1>\
             <p>Run <code>cargo build --release -p hapi-hub</code> to auto-build, \
             or <code>cd ../hapi-ts/web &amp;&amp; bun run build</code> for dev mode.</p>",
        ),
    )
        .into_response()
}
...
async fn serve_from_disk(path: &str) -> Option<Response> {
    ...
    // Candidate directories: relative to cwd and to crate root
    let candidates = ["web/dist", "../../web/dist", "../hapi-ts/web/dist"];
